...
class Fiery(nn.Module):
    ...
    def forward(self, image, intrinsics, extrinsics, future_egomotion, future_distribution_inputs=None, noise=None):
        ...
        print("N_futures:", self.n_future)
        print("Probabilistic enabled:", self.cfg.PROBABILISTIC.ENABLED)
        
        outputs = []
        outputs_np = []
        
        for i in range(1):  # NB! changed from 100
            ...
            if self.n_future > 0:
                extra_shape = future_states.shape
            else:
                extra_shape = states[:, -1:].shape
                
            print("States.shape", states.shape)
            print("Extra shape:", extra_shape)
            
            outputs.append(output)
            outputs_np.append({"segmentation":output["segmentation"].cpu().detach().numpy(),
                                'present_mu':output["present_mu"].cpu().detach().numpy(),
                                'present_log_sigma':output["present_log_sigma"].cpu().detach().numpy(),
                                'future_mu':output["future_mu"].cpu().detach().numpy(),
                                'future_log_sigma':output["future_log_sigma"].cpu().detach().numpy(),
                                'instance_center':output["instance_center"].cpu().detach().numpy(), 
                                'instance_offset':output["instance_offset"].cpu().detach().numpy(), 
                                'instance_flow':output["instance_flow"].cpu().detach().numpy()})

        return outputs, outputs_np
    ...
    def distribution_forward(self, present_features, future_distribution_inputs=None, noise=None):
        ...
        if noise is None:
            ...
            print(torch.tensor(10*np.ones_like([32]), device=present_mu.device).shape)
            print(noise.shape)
            noise = torch.tensor(10*np.ones_like([32]), device = present_mu.device)*noise  # NB! muudetud TODO
            #else:
        ...
        sample = mu  # + sigma* noise  # Noise deactivated NB!
