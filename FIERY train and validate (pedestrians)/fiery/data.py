...
class FuturePredictionDataset(torch.utils.data.Dataset):
    ...
    def get_scenes(self):
        ...
        if self.is_lyft:
            ...
            split = {'v1.0-trainval': {True: 'train', False: 'val'},
                     ...
                     'v1.0-test': {True: 'test', False: 'test'},}[  # Added this row, shouldn't train on test though..
                self.nusc.version
            ][self.is_train]
    ...
    def get_birds_eye_view_label(self, rec, instance_map):
        ...
        for annotation_token in rec['anns']:
            ...
            if not self.is_lyft:
                # NuScenes filter
                if self.cfg.TRAIN_ON_PEDESTRIANS:
                    if 'pedestrian' not in annotation['category_name']:
                        continue
...
def prepare_dataloaders(cfg, return_dataset=False):
    ...
    if cfg.DATASET.NAME == 'nuscenes':
        # 28130 train and 6019 val and test?
